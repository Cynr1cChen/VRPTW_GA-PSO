...
def global_result_from_experiments(df_experiment_result_recorder, dict_routes, no_experiments):
    # 找到最小的 fitness 对应的实验索引
    min_fitness_experiment = df_experiment_result_recorder['fitness'].idxmin()
    # 创建结果数据框
    df_results = pd.DataFrame(columns=['num_vehicles', 'distance', 'fitness', 'average_time'])
    # 取出对应的结果
    df_results.loc[0, ['num_vehicles', 'distance', 'fitness', 'average_time']] = [
        df_experiment_result_recorder.loc[min_fitness_experiment, 'num_vehicles'],
        df_experiment_result_recorder.loc[min_fitness_experiment, 'distance'],
        df_experiment_result_recorder.loc[min_fitness_experiment, 'fitness'],
        df_experiment_result_recorder['time'].mean()
    ]
    # 提取对应的路线
    dict_final_route = {}
